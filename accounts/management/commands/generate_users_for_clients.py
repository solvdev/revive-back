...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for client in clients:
            try:
                if dry_run:
                    self.stdout.write(f"DRY RUN: Generaría usuario para {client.first_name} {client.last_name} ({client.email})")
                    continue
                ...
                self.stdout.write(f"✓ Usuario creado para {client.first_name} {client.last_name} - Username: {username}")
    ...
    def send_notification_emails(self, generated_users):
        ...
        try:
            ...
            frontend_url = getattr(settings, 'FRONTEND_URL', 'https://app.vilepilates.com')
        ...
        for user_data in generated_users:
            ...
            try:
                ...
                self.stdout.write(self.style.ERROR(f"Error enviando email a {user.email}: {str(e)}"))
